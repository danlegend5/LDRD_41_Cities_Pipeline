...
# measurements created by stage 0. In particular, for the following cities, all of the detectors and their
# measurements are rejected:
#
# birmingham <= The loop detector measurements are flow and speed (where speed has been inferred in an
#               unclear way; 66 loop detectors)
# bolton <= The loop detector measurements are flow and speed (where speed has been inferred in an unclear
#           way; 166 loop detectors)
# duisburg <= The data cannot be analysed because an NDA is in force (190 loop detectors)
# frankfurt <= Too few loop detector measurements are available because the data come from a single day
#              (112 loop detectors)
# groningen <= The data come from a mixture of loop detectors and bluetooth detectors (55 detectors)
# innsbruck <= The loop detector measurements are flow and speed (where speed has been inferred in an
#              unclear way; 16 loop detectors)
# manchester <= The loop detector measurements are flow and speed (where speed has been inferred in an
#               unclear way; 181 loop detectors)
# melbourne <= The data come from a mixture of loop detectors and bluetooth detectors (1,630 detectors)
# munich <= Too few loop detector measurements are available because the data come from a single day (520
#           loop detectors)
# paris <= The aggregation time-interval is very long (1 hour; 247 loop detectors)
# rotterdam <= The loop detector measurements are flow and speed (where speed has been inferred in an
#              unclear way; 259 loop detectors)
# torino <= The loop detector measurements are flow and speed (where speed has been inferred in an unclear
#           way; 399 loop detectors)
# utrecht <= The data come from a mixture of loop detectors and bluetooth detectors (1,072 detectors)
# vilnius <= Too few loop detector measurements are available because the data come from a single day
#            (581 loop detectors)
#
# This script also further reorganises the data based on the data provenance.
#
# N.B: The source of the "detector" measurements for each city is listed below (data provenance). Data for
#      a city come from one of four possible sources:
#
...
output_dir_ld_locations = os.path.join(config.output_dir, 's1b.Loop.Detector.Locations')
print('')
...
output_dir_ld_measurements_raw = os.path.join(config.output_dir, 's1b.Loop.Detector.Measurements.Raw')
...
output_dir_ld_measurements_arima = os.path.join(config.output_dir, 's1b.Loop.Detector.Measurements.ARIMA')
...
for file in file_list:
    ...
    # Reject all of the loop detectors for the current city if the city is in the list: 'birmingham', 'bolton',
    # 'duisburg', 'frankfurt', 'groningen', 'innsbruck', 'manchester', 'melbourne', 'munich', 'paris', 'rotterdam',
    # 'torino', 'utrecht', 'vilnius'
    if city_name in ['birmingham', 'bolton', 'duisburg', 'frankfurt', 'groningen', 'innsbruck', 'manchester', 'melbourne', 'munich',
                     'paris', 'rotterdam', 'torino', 'utrecht', 'vilnius']:
        print('WARNING - There are no loop detectors with acceptable data for: ' + city_name)
        continue

    # Determine the source of the flow and occupancy measurements
    if city_name in ['constance', 'essen']:
    ...
    # 'other', 'path', 'pedestrian' (133 loop detectors rejected)
    ...
    # Filter out the loop detectors with bad "NLANES" values (4 loop detectors rejected)
    nld_locations_old = nld_locations
    ...
    for i in range(nld_locations):
        ...
        # If the data source consists of flow and occupancy measurements, and the speed measurements have been
        # inferred
        else:

            # Ensure that the loop detector measurements (raw) with bad "FLOW" or bad "OCCUPANCY" or bad "SPEED"
            # values are flagged with "ERROR_FLAG = 1"
        ...
        if ngood < config.min_ngood_ld_measurements:
            selection[i] = False
            nrejected_reason4 += 1
            continue
        ...
        if data_source == 'LD.Flow.LD.Occupancy':
            ...
            # "ARIMA_FLOW" or bad "ARIMA_OCCUPANCY" values are flagged with "ERROR_FLAG = 1", and delete the
            # "SPEED" and "ARIMA_SPEED" columns
        ...
        # If the number of good loop detector measurements (ARIMA) for the current loop detector is less than
        # the minimum required, then move on to the next loop detector
        ...
        if ngood < config.min_ngood_ld_measurements: continue
    ...
    print('No. of loop detectors rejected because the number of good loop detector measurements (raw) is less than the minimum required:        ' + str(nrejected_reason4))
