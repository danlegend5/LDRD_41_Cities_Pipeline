...
# This script converts the filtered loop detector locations and measurements data created by stage 2b into
# an efficient two-dimensional data format for subsequent analyses.
...
output_dir_ld_data = os.path.join(config.output_dir, 's3b.Loop.Detector.Data')
print('')
...
# been produced in stage 2b
print('Copying across the loop detector locations data files and the summary statistics text files from stage 2b...')
file_list = glob.glob(os.path.join(config.output_dir, 's2b.Loop.Detector.Locations', 'detectors.LD.Flow.LD.*.fits'))
for file in file_list: shutil.copy(file, output_dir_ld_data)
shutil.copy(os.path.join(config.output_dir, 's2b.Loop.Detector.Locations', 'summary.statistics.of.detectors.per.city.txt'), output_dir_ld_data)
shutil.copy(os.path.join(config.output_dir, 's2b.Loop.Detector.Locations', 'summary.statistics.of.measurements.per.city.detector.txt'), output_dir_ld_data)
...
for file in file_list:
    ...
    occ_im = numpy.zeros((nld_locations, ntime_bins), dtype = numpy.float64) - 1.0
    error_im = numpy.ones((nld_locations, ntime_bins), dtype = numpy.int32)
    if data_source == 'LD.Flow.LD.Occupancy.LD.Speed': speed_im = numpy.zeros((nld_locations, ntime_bins), dtype = numpy.float64) - 1.0
    arima_flow_im = numpy.zeros((nld_locations, ntime_bins), dtype = numpy.float64) - 1.0
    arima_occ_im = numpy.zeros((nld_locations, ntime_bins), dtype = numpy.float64) - 1.0
    arima_error_im = numpy.ones((nld_locations, ntime_bins), dtype = numpy.int32)
    if data_source == 'LD.Flow.LD.Occupancy.LD.Speed': arima_speed_im = numpy.zeros((nld_locations, ntime_bins), dtype = numpy.float64) - 1.0
    arima_data_tag = 0
    for i in range(nld_locations):
        ...
        ld_measurements_file_raw = os.path.join(config.output_dir, 's2b.Loop.Detector.Measurements.Raw', data_source, country_name, city_name, curr_detector_id, ld_measurements_file_raw)
        ...
        for j in range(nld_measurements_raw):
            ...
            occ_im[i, curr_time_bin_sub] = ld_measurements_table_raw['OCCUPANCY'][j]
            error_im[i, curr_time_bin_sub] = 0
            if data_source == 'LD.Flow.LD.Occupancy.LD.Speed': speed_im[i, curr_time_bin_sub] = ld_measurements_table_raw['SPEED'][j]

        # Determine the full directory path and filename of the corresponding loop detector measurements
        # data file (ARIMA)
        ld_measurements_file_arima = 'measurements.ARIMA.' + data_source + '.' + country_name + '.' + city_name + '.' + curr_detector_id + '.fits'
        ld_measurements_file_arima = os.path.join(config.output_dir, 's2b.Loop.Detector.Measurements.ARIMA', data_source, country_name, city_name, curr_detector_id, ld_measurements_file_arima)
        ...
        for j in range(nld_measurements_arima):
            ...
            arima_occ_im[i, curr_time_bin_sub] = ld_measurements_table_arima['ARIMA_OCCUPANCY'][j]
            arima_error_im[i, curr_time_bin_sub] = 0
            if data_source == 'LD.Flow.LD.Occupancy.LD.Speed': arima_speed_im[i, curr_time_bin_sub] = ld_measurements_table_arima['ARIMA_SPEED'][j]
